...
socket.on("outhers_1_1", (message) => {
    console.log("others",message.userId1, message.userId2)
});

socket.on("request_refusal", (message) => {
    alert("상대방이 1대1 대화를 거절하였습니다.")
});

socket.on("request_accept", (message) => {
    alert("상대방이 1대1 대화를 수락하였습니다.")
});