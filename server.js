...
io.on('connection', function(socket) {
    ...
    socket.on("meeting_disconnect", () => {
        try {
            ...
            //if(shareUserId[roomId] === socket.id && roomLeader === socket.id)  share_disconnect(shareUserId[roomId])  //공유자가 나간 경우 공유 끊음
            ...
            if(roomLeader !== socket.id) {
                closeSenderPCs(socket.id, 'meeting');
                ...
                for(var key in roomList[roomId]) {
                    ...
                    delete userStreams['meeting'][key];
                }
    ...
    socket.on('share_disconnect', () => {
        ...
        try{
            ...
            console.log("share_disconnect에서 에러");
            console.error(e)
            console.log("shareSwitch",shareSwitch);
        }

        console.log("sendPCs['share']",sendPCs['share']);
        console.log("receivePCs['share']",receivePCs['share']);
        console.log("userStreams['share']",userStreams['share']);
    });
    ...
    socket.on('request_1_1', (message) => {
        ...
        for(var key in roomList[message.roomId]) {
            if(key === message.target) {
                target = key;
            }
        }
        io.to(target).emit('get_1_1_request', {userName: message.userName, userId:message.socketId});
    });
    ...
    socket.on('accept_1_1', (message) => {  
        for(var key in roomList[message.roomId]) {
            if(key === message.target) {
                ...
                ;//다른 유저들이 화면 공유중인 사람들의 화면을 사라지게 해주기
            }
        }
    ...
    socket.on('refusal_1_1', (message) => {
        io.to(message.target).emit('request_refusal');
    });
    ...
    socket.on("show",function (data){  //확인용
        ...
        console.log("해당 socket의 userStream:", userStreams['meeting'][socket.id]);
    });
...
function share_disconnect(socketId){
    try{
        console.log("함수실행!!!")
        if(!shareSwitch[users[socketId]['room_id']]) return;
        shareSwitch[users[socketId]['room_id']] =false;
        
        receivePCs['share'][socketId].close();
        delete receivePCs['share'][socketId];

        for(var key in roomList[users[socketId]['room_id']]) {
            if(!sendPCs['share'][socketId][key]) continue;

            sendPCs['share'][socketId][key].close();
            delete sendPCs['share'][socketId][key];

            if(!userStreams['share'][socketId][key]) continue;

            delete userStreams['share'][socketId][key];

        }
        delete sendPCs['share'][socketId];
        delete userStreams['share'][socketId]
        socket.broadcast.to(users[socketId]['room_id']).emit('share_disconnect');

        //delete shareSwitch[users[socket.id]['room_id']];
        
        delete shareUserId[users[socketId]['room_id']];
    }
    catch(e){
        console.log("함수에러!!!")
        console.error(e)
